...
package com.alibaba.nacos.plugin.auth.impl.persistence.repository.embedded;

import com.alibaba.nacos.common.utils.ExceptionUtil;
import com.alibaba.nacos.common.utils.LoggerUtils;
import com.alibaba.nacos.plugin.auth.impl.persistence.sql.ModifyRequest;
import com.alibaba.nacos.plugin.auth.impl.utils.DerbyUtils;
...
public interface BaseDatabaseOperate extends DatabaseOperate {
    ...
    default <R> R queryOne(JdbcTemplate jdbcTemplate, String sql, Class<R> cls) {
        try {
            ...
            LogUtil.AUTH.error("[db-error] can't get connection : {}", ExceptionUtil.getAllExceptionMsg(e));
            throw e;
        } catch (DataAccessException e) {
            LogUtil.AUTH.error("[db-error] DataAccessException : {}", ExceptionUtil.getAllExceptionMsg(e));
            throw e;
        }
    }
    ...
    default <R> R queryOne(JdbcTemplate jdbcTemplate, String sql, Object[] args, Class<R> cls) {
        try {
            ...
            LogUtil.AUTH.error("[db-error] {}", e.toString());
            throw e;
        } catch (DataAccessException e) {
            LogUtil.AUTH.error("[db-error] DataAccessException sql : {}, args : {}, error : {}", sql, args,
        ...
        return transactionTemplate.execute(status -> {
            ...
            try {
                contexts.forEach(pair -> {
                    ...
                    args[0] = pair.getArgs();
                    LoggerUtils.printIfDebugEnabled(LogUtil.AUTH, "current sql : {}", errSql[0]);
                    LoggerUtils.printIfDebugEnabled(LogUtil.AUTH, "current args : {}", args[0]);
                ...
                LogUtil.AUTH.error("[db-error] sql : {}, args : {}, error : {}", errSql[0], args[0], e.toString());
                if (consumer != null) {
                ...
                LogUtil.AUTH.error("[db-error] DataAccessException sql : {}, args : {}, error : {}", errSql[0], args[0],
                        ExceptionUtil.getAllExceptionMsg(e));
                throw e;
            }
        });
